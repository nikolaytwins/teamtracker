...
export function ensureAgencyProjectsColumns(db: Database.Database) {
  ...
  try {
    db.exec(`ALTER TABLE AgencyProject ADD COLUMN workStatus TEXT NOT NULL DEFAULT 'not_started'`);
  } catch {
    /* column exists */
  }
  try {
    db.exec(`ALTER TABLE AgencyProject ADD COLUMN kanbanSortOrder INTEGER NOT NULL DEFAULT 0`);
  } catch {
    /* column exists */
  }
  try {
    db.exec(`ALTER TABLE AgencyProject ADD COLUMN hourlyRateRub REAL NOT NULL DEFAULT 0`);
  } catch {
    /* column exists */
  }
  try {
