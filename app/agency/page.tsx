...
export default function AgencyPage() {
    ...
    try {
      ...
      const projectsWithExpenses = await Promise.all(
        filteredProjects.map(async (p: Project) => {
          try {
            const expensesRes = await fetch(apiUrl(`/api/agency/expenses?projectId=${p.id}`))
            ...
            try {
              const detailsRes = await fetch(apiUrl(`/api/agency/project-details?projectId=${p.id}`))
            ...
            if (p.status === 'paid' && p.paidAmount !== effectiveTotalAmount) {
              ...
              try {
                await fetch(apiUrl(`/api/agency/projects/${p.id}`), {
                  method: 'PUT',
  ...
  const handleDeleteExpense = async (id: string) => {
    ...
    try {
      const res = await fetch(apiUrl(`/api/agency/general-expenses/${id}`), {
        method: 'DELETE',
      })
  ...
  const handleDeleteProject = async (id: string) => {
    ...
    try {
      const res = await fetch(apiUrl(`/api/agency/projects/${id}`), {
        method: 'DELETE',
      })
      if (!res.ok) {
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              onClick={async () => {
                ...
                try {
                  const res = await fetch(apiUrl('/api/agency/general-expenses/copy-from-month'), {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                      fromYear: prevYear,
