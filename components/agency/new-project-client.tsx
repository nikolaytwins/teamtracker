...
export function NewAgencyProjectClient({ variant }: { variant: AgencyFinanceVariant }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch(apiUrl(`${paths.apiBase}/projects`), {
        method: 'POST',
      ...
      if (res.ok) {
        router.push(appPath(paths.listHref))
        router.refresh()
      } else {
  ...
  return (
    <div>
      <div className="mb-6">
        <Link href={appPath(paths.listHref)} className="text-sm font-medium text-[var(--primary)] hover:underline">
          ← Назад к проектам
        </Link>
      </div>
