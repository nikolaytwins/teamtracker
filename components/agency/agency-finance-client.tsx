...
function MoveProjectPicker({
  ...
  const handleMove = async (year: number, month: number) => {
    setMoving(true)
    try {
      const res = await fetch(apiUrl(`${apiBase}/projects/${projectId}/move-to-month`), {
        method: 'POST',
...
export function AgencyFinanceClient({ variant }: { variant: AgencyFinanceVariant }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch(apiUrl(`${apiBase}/projects`))
    ...
    try {
      ...
      const projectsWithExpenses = await Promise.all(
        filteredProjects.map(async (p: Project) => {
          try {
            const expensesRes = await fetch(apiUrl(`${apiBase}/expenses?projectId=${p.id}`))
            ...
            if (p.status === 'paid' && p.paidAmount !== effectiveTotalAmount) {
              ...
              try {
                await fetch(apiUrl(`${apiBase}/projects/${p.id}`), {
                  method: 'PUT',
                  headers: { 'Content-Type': 'application/json' },
  ...
  const fetchGeneralExpenses = useCallback(async () => {
    if (!monthFilterReady) return
    try {
      const res = await fetch(apiUrl(`${apiBase}/general-expenses`))
      const data = await res.json()
  ...
  const handleDeleteExpense = async (id: string) => {
    ...
    try {
      const res = await fetch(apiUrl(`${apiBase}/general-expenses/${id}`), {
        method: 'DELETE',
      })
  ...
  const handleDeleteProject = async (id: string) => {
    ...
    try {
      const res = await fetch(apiUrl(`${apiBase}/projects/${id}`), {
        method: 'DELETE',
      })
      if (!res.ok) {
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex space-x-3">
          <Link
            href={appPath(paths.newProjectHref)}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      onClick={async (e) => {
                        e.preventDefault()
                        try {
                          const res = await fetch(apiUrl(`${apiBase}/projects/${project.id}/copy`), {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
            ...
            {projects.length === 0 && (
              <tr>
                <td colSpan={8} className="px-3 py-4 sm:px-6 text-center text-sm text-[var(--muted-foreground)]">
                  Нет проектов. <Link href={appPath(paths.newProjectHref)} className="text-[var(--primary)] hover:underline">Создать первый проект</Link>
                </td>
              </tr>
            )}
          </tbody>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              onClick={async () => {
                ...
                try {
                  const res = await fetch(apiUrl(`${apiBase}/general-expenses/copy-from-month`), {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                      fromYear: prevYear,
