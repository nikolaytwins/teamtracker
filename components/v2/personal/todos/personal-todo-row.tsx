...
function PersonalTodoRowContent({
  todo,
  onToggle,
  onOpen,
  onAddSubtask,
  ...
  const inlineSubtasks = todo.subtasks ?? [];
  const hasSubtasks = inlineSubtasks.length > 0 || (todo.subtask_count ?? 0) > 0;
    ...
    <>
      <div
        ...
        className={`group relative flex cursor-pointer items-center gap-3 transition-colors duration-200 ${
          nested ? "py-2 pl-3 pr-3" : compact ? "px-2 py-2" : "py-3 pl-4 pr-3"
        ...
        <div className="min-w-0 flex-1">
          <h4
            className={`v2-tight truncate font-medium text-[var(--v2-ink-900)] ${
              nested ? "text-[13px]" : compact ? "text-[13px]" : "text-[14.5px]"
          ...
          {!nested ? (
            <div className="mt-1.5 flex flex-wrap items-center gap-2 text-[12px] text-[var(--v2-ink-500)]">
              ...
              {todo.priority ? (
                <>
                  {(showProject && todo.project_name) || dateLabel ? (
                    <span className="text-[var(--v2-ink-300)]">·</span>
                  ) : null}
                  <PriorityDot priority={todo.priority} />
                </>
              ) : null}
              {hasSubtasks && !inlineSubtasks.length ? (
                <>
                  <span className="text-[var(--v2-ink-300)]">·</span>
      ...
      {inlineSubtasks.length > 0 ? (
        <div className="border-t border-[var(--v2-ink-100)]/60 bg-[var(--v2-ink-50)]/40">
          {inlineSubtasks.map((sub) => (
            <div key={sub.id} className="border-l-2 border-[var(--v2-ink-200)]/80 pl-6">
              <PersonalTodoRowContent
                todo={sub}
                onToggle={onToggle}
                onOpen={onOpen}
                showProject={false}
...
export function PersonalTodoRowItem(props: RowProps) {
  return <PersonalTodoRowContent {...props} />;
}
