...
export function PersonalIncomeChart({
  ...
  const padBot = 34;
  const [hover, setHover] = useState(() => Math.max(0, data.length - 1));
  const vals = data.map((d) => d.earned_rub);
  if (vals.length < 2) return null;
  const rawMin = Math.min(...vals);
  const rawMax = Math.max(...vals);
  const span = rawMax - rawMin || Math.max(Math.abs(rawMax), 1) * 0.1 || 1;
  const min = rawMin - span * 0.02;
  const max = rawMax + span * 0.02;

  const X = (i: number) => padX + (i * (W - padX * 2)) / (data.length - 1);
  const Y = (v: number) => padTop + (1 - (v - min) / (max - min)) * (H - padTop - padBot);
  const pts = data.map((d, i) => ({ x: X(i), y: Y(d.earned_rub) }));
  const line = smoothPath(pts);
  const area = `${line} L ${pts[pts.length - 1]!.x} ${H - padBot} L ${pts[0]!.x} ${H - padBot} Z`;
  const hp = pts[hover];
  const hd = data[hover];
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id="pfIncFill" x1="0" y1="0" x2="0" y2="1">
      ...
      <path d={area} fill="url(#pfIncFill)" />
...
/** @deprecated — оставлен для совместимости, используйте PersonalIncomeChart */
export function PersonalIncomeBars(props: {
  data: PersonalMonthSnapshotRow[];
  masked: boolean;
  currentYear: number;
  currentMonth: number;
}) {
  return <PersonalIncomeChart {...props} />;
}
